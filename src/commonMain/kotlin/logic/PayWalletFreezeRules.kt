...
object PayWalletFreezeRules {
    ...
    fun ensureCanFreeze(balance: Long, freezePrice: Long, amount: Long) {
        require(amount > 0) { "freeze amount must be positive: $amount" }
        require(available(balance, freezePrice) >= amount) {
    ...
    fun ensureCanUnfreeze(freezePrice: Long, amount: Long) {
        require(amount > 0) { "unfreeze amount must be positive: $amount" }
        require(freezePrice >= amount) {
    ...
    fun ensureCanDeductFrozen(balance: Long, freezePrice: Long, amount: Long) {
        require(amount > 0) { "deduct amount must be positive: $amount" }
        require(freezePrice >= amount) {
